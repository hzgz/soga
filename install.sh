...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/hzgz/soga/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 soga 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 soga 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 soga 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/hzgz/soga/releases/download/${last_version}/soga-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/hzgz/soga/releases/download/${last_version}/soga-linux-${arch}.tar.gz"
    ...
    curl -o /usr/bin/soga -Ls https://raw.githubusercontent.com/hzgz/soga/master/soga.sh
    chmod +x /usr/bin/soga
    curl -o /usr/bin/soga-tool -Ls https://raw.githubusercontent.com/hzgz/soga/master/soga-tool-${arch}
